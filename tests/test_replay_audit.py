...
@pytest.mark.parametrize(
    ...
    [
        (0.20, AuditVerdict.RELIABLE),
        (0.02, AuditVerdict.UNCERTAIN),
        (-0.20, AuditVerdict.HARMFUL),
    ],
)
def test_audit_compares_target_against_repeat_draw_value(
    ...
    search = SearchResult(
        (
            ...
            MoveStatistics(alternative, 4, 0.4, alternative_value),
        ),
        0.0,
        16,
    )
    ...
    assert audit.repeat_visit_mass == pytest.approx(0.75)
    assert audit.target_visit_mass == pytest.approx(0.25)
    assert audit.target_mcts_value == pytest.approx(alternative_value)
    assert audit.champion_selected_repeats
    assert not audit.target_contains_champion_selection
