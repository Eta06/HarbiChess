...
def test_prepared_model_quality_dataset_preserves_metrics() -> None:
    rules, game = scripted_game()
    records = records_from_game(game, run_id="quality-cache", rules=rules)
    network = HarbiChessNetwork(
        NetworkConfig(
            trunk_channels=8,
            residual_blocks=1,
    ...
    prepared = prepare_model_quality_dataset(records, batch_size=2, rules=rules)

    uncached = evaluate_model_quality(network, records, batch_size=2, rules=rules)
    cached = evaluate_model_quality(network, prepared, rules=rules)
