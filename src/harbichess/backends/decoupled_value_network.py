...
class HarbiChessDecoupledValueNetwork(HarbiChessInvariantValueNetwork):
    ...
    def __init__(
        ...
        *,
        ...
        self.material_value_linear = nn.Linear(
            PIECE_PLANES_PER_STEP + METADATA_PLANES, 1
        )
        self.material_value_linear.weight = mx.zeros_like(
            self.material_value_linear.weight
        )
    ...
    def from_base(
        ...
        *,
        ...
        target.material_value_linear.weight = mx.zeros_like(
            target.material_value_linear.weight
        )
        target.material_value_linear.bias = mx.zeros_like(
            target.material_value_linear.bias
        )
    ...
    def material_value(self, inputs: mx.array) -> mx.array:
        return mx.tanh(self.material_value_linear(self.invariant_features(inputs))).squeeze(
            axis=1
        )
